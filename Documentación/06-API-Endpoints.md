...
| `mp-oauth-start` | Inicia OAuth de MercadoPago para conectar la cuenta del **profesor** (payouts); genera `state` anti-CSRF en `mp_oauth_states` y devuelve la URL de autorización. **⚠️ Huérfana desde el 16-ago**: ningún componente del frontend la invoca ya (el desembolso al profesor pasó a resolverse vía `refund_methods` + Fintoc, no MercadoPago Connect) — queda desplegada pero sin uso, pendiente de decisión de limpieza |
| `mp-oauth-callback` | Callback OAuth del profesor (sin JWT); valida `state`, canjea code→tokens y guarda la cuenta del vendedor. Mismo estado huérfano que `mp-oauth-start` |
