...
> ⚠️ **Documento histórico — análisis puntual con fecha de corte.** Este informe
> fue escrito en un momento dado y **no se actualiza automáticamente** con cada
> cambio de código. El resto del documento describe el estado **al momento del
> análisis original**; antes de actuar sobre cualquier hallazgo, verifícalo
> contra el código actual (no asumas que sigue vigente ni que ya se corrigió).
>
> **Re-verificación del 19-jul-2026** (contra el código real, no contra este
> documento): de los hallazgos críticos y altos, **G-06, G-07 y G-16 ya fueron
> corregidos** (ver `11-Mejoras-Incorporadas.md`). De los que no se habían
> re-confirmado, el resultado es:
>
> | Hallazgo | Estado real (19-jul-2026) |
> |---|---|
> | G-04 (firma HMAC opcional) | **Ya no aplica** — rediseño intencional: la verificación HMAC es no bloqueante a propósito; la seguridad real recae en validar el pago contra la API de MercadoPago (`mercadopago-webhook/index.ts`) |
> | G-05 (idempotencia webhook) | **Matizado** — mitigado para inscripciones (constraint `enrollments_unique_beneficiary`), **sigue vigente** para el flujo `ROOM_RESERVATION` y para el `UPDATE payment_sessions`/`audit_logs` sin guard |
> | G-08 (`payments.FAILED` nunca se escribe) | **Corregido** — `process-refunds/index.ts` escribe `FAILED` con guard atómico tras reintentos agotados |
> | G-09 (sin vía de cancelación) | **Matizado** — `enrollments.status='CANCELLED'` ya se escribe (cron de reagendamiento); `classes.status='CANCELLED'` **sigue sin ningún escritor** en el código |
> | G-10 (condición de carrera liberar/reembolsar) | **No aplica** — los `UPDATE ... WHERE status='RETAINED'` ya son atómicos por fila; no hay TOCTOU real |
> | G-11 / G-12 (regeneración sin dedupe / re-disponibiliza salas ocupadas) | **Corregidos** — `UNIQUE(room_id, start_time)` real + `ON CONFLICT DO NOTHING` (`20260623000000_fix_schedule_timezone.sql`) |
> | G-13 (reagendar sin sincronizar bloques) | **Matizado** — corregido en el mecanismo vigente (`teacher-reschedule-class`, `sede-reschedule-class`); **sigue vigente** en el mecanismo viejo `teacher-decision`, que hoy está huérfano de UI (ver `02-Reglas-de-Negocio.md` R13/R18) |
> | G-14 (timeout no revierte horario) | **Sigue vigente**, mismo mecanismo huérfano (`teacher-decision` + cron `process_reschedule_timeouts`) |
> | G-15 (`propose-reschedule` no valida disponibilidad) | **Sigue vigente**, mismo mecanismo huérfano |
> | G-19 (RLS `WITH CHECK (true)` en `reschedules` INSERT) | ✅ **Corregido el 19-jul-2026** (`20260719000000_fix_rls_reschedule_and_status_history.sql`): se retiró la política `resched_insert_auth`. La escritura legítima (Edge Function `propose-reschedule`) usa `service_role` y no dependía de esta política, así que no se rompe nada |
> | G-20 (RLS `WITH CHECK (true)` en `class_status_history` INSERT) | ✅ **Corregido el 19-jul-2026** (misma migración): se retiró la política `csh_insert_system`. El único escritor legítimo, el trigger `track_class_status()` (`SECURITY DEFINER`), bypasea RLS igual |
>
> Al momento de la re-verificación (antes de esta corrección), G-19 y G-20 eran
> las dos únicas brechas de esta lista con un riesgo de seguridad activo y
> explotable (no solo teórico) — el resto ya estaba corregido, o describe un
> mecanismo (`propose-reschedule`/`teacher-decision`) que ya no tiene ningún
> punto de entrada desde la UI del frontend.
>
> **Alcance original:** solo lectura sobre el código de producción y consultas de solo
> lectura al backend hosteado mediante el power `supabase-hosted`. No se modificó
...
4. **Corregir las políticas RLS permisivas y la cadena de agendamiento** (~~G-19,
   G-20~~ ✅ corregidas 19-jul-2026, G-11/G-12 ✅ corregidas, G-13/G-14/G-15
   siguen vigentes pero en un mecanismo huérfano de UI — ver re-verificación al
...
> ⚠️ **Inventario desactualizado.** Esta subsección listaba 15 Edge Functions al
> momento del análisis original. El repositorio real, a la fecha de la
> re-verificación (19-jul-2026), tiene **27 Edge Functions** (28 directorios en
> `supabase/functions/` menos `_shared/`). Faltan en esta lista:
> `mp-connect-start`, `mp-connect-callback`, `mp-oauth-start`, `mp-oauth-callback`,
> `process-payouts`, `process-refunds`, `admin-payments`, `admin-metrics`,
> `ga-metrics`, `save-attendance`, `reserve-room-preference`,
> `sede-reschedule-class`, `teacher-reschedule-class`. El detalle actualizado y
> mantenido de cada una vive en `06-API-Endpoints.md` — no se duplica aquí para
> evitar que este documento vuelva a desincronizarse.

Inventario original (15 Edge Functions) de `supabase/functions/` (se excluye el
...
**Conteo observado (original, desactualizado):** 15 Edge Functions, coincidente
con las 15 esperadas en el momento del análisis. **Conteo real (19-jul-2026):**
27 Edge Functions — ver la nota al inicio de esta subsección y
`06-API-Endpoints.md` para el inventario completo y mantenido. De las nuevas,
solo `mercadopago-webhook` mantiene `verify_jwt = false` (sin cambios); todas
las demás, incluidas las 13 agregadas después del análisis original, exigen
JWT. La presencia y el `verify_jwt` reales de las funciones desplegadas se
contrastan en el capítulo 5 (Drift) — ese contraste tampoco se ha vuelto a
correr desde el análisis original.
