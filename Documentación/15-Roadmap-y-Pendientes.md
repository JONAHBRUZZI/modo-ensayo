...
| Ítem | Detalle | Dónde |
|---|---|---|
| Desembolso real a profesores | **Implementado (16-ago), sin probar.** `process-payouts` ya usa `FintocPayoutProvider` (transferencia directa a `refund_methods`, firma JWS RS256) en vez del stub de MercadoPago. El trigger de publicación ya exige "datos bancarios cargados" en vez de "MercadoPago conectado". **Falta:** (1) el usuario debe generar el par de llaves JWS (`openssl genrsa`...) y subir la pública al dashboard de Fintoc, (2) `supabase secrets set FINTOC_SECRET_KEY / FINTOC_JWS_PRIVATE_KEY`, (3) `supabase db push` + `functions deploy process-payouts create-class`, (4) probar con un payout de monto mínimo real, (5) revisar el mapeo banco→`institution_id` en `fintocPayoutProvider.ts` — solo cubre los bancos chilenos más comunes | `supabase/functions/process-payouts/`, `_shared/fintocPayoutProvider.ts` |
