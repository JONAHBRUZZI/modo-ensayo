...
public interface RoomAvailabilityRepository extends JpaRepository<RoomAvailability, UUID> {
    ...
    List<RoomAvailability> findByRoomIdAndEndTimeAfter(UUID roomId, Instant now);
    List<RoomAvailability> findByRoomIdAndStartTimeBeforeAndEndTimeAfter(UUID roomId, Instant start, Instant end);
}
