<template>
  <div>
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-white mb-2">Clases Disponibles</h1>
      ...
      <div v-for="cls in classes" :key="cls.id"
        ...
        <div class="h-24 bg-gradient-to-r from-indigo-600 to-purple-600 relative">
          <div class="absolute -bottom-6 left-6">
            <div class="w-12 h-12 bg-[#161824] rounded-xl border border-white/10 flex items-center justify-center">
        ...
        <div class="pt-8 pb-6 px-6">
          ...
          <span class="inline-block px-3 py-1 bg-indigo-500/20 text-indigo-400 text-xs font-medium rounded-full border border-indigo-500/30 mb-4">
          ...
          <div class="flex items-center justify-between mb-4">
            <div>
              <span class="text-2xl font-bold text-white">${{ cls.price?.toLocaleString() }}</span>
              <span class="text-gray-500 text-sm">/clase</span>
            </div>
            <span v-if="cls.capacity <= 3" class="text-red-400 text-sm font-medium">
    ...
    <div v-else class="text-center py-20">
      ...
      <h3 class="text-lg font-semibold text-white mb-2">No hay clases disponibles</h3>
      <p class="text-gray-500">Vuelve pronto para ver nuevas clases</p>
    </div>

    <!-- Beneficiary Modal -->
    <div v-if="showBeneficiaryModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div class="w-full max-w-md bg-[#161824] rounded-2xl border border-white/10 p-6 shadow-2xl mx-4">
        ...
        <div v-if="associates.length === 0" class="mb-4 rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 text-sm text-amber-400">
          No tienes asociados vinculados.
          <router-link to="/associates" class="font-medium text-indigo-400 hover:underline ml-1">Vincular asociados</router-link>
        </div>

        <div class="flex justify-end gap-2">
          <button @click="showBeneficiaryModal = false"
          ...
          <button @click="confirmAddToCart"
                  ...
                  class="rounded-lg bg-indigo-600 px-4 py-2 text-sm text-white font-medium hover:bg-indigo-500 disabled:opacity-50 transition-colors">
            Agregar
          </button>
        </div>
      </div>
    ...
    <Transition enter-active-class="transform ease-out duration-300 transition"
      ...
      <div v-if="showToast"
           class="fixed bottom-4 right-4 z-50 bg-emerald-500 text-white px-6 py-4 rounded-xl shadow-lg shadow-emerald-500/25 flex items-center gap-3">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
        <div>
          <p class="font-semibold text-sm">¡Agregado al carrito!</p>
          <p class="text-xs text-emerald-100">{{ toastMessage }}</p>
        </div>
      </div>
    </Transition>
  </div>
