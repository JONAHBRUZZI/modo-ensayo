<template>
  <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <!-- Datos bancarios (requeridos para publicar clases y cobrar) -->
    <div class="card mb-8">
      <h3 class="text-sm font-semibold text-primary uppercase tracking-wider mb-3">Cobros — Datos bancarios</h3>
      <div v-if="tieneDatosBancarios" class="flex items-center gap-3 flex-wrap">
        <span class="inline-flex items-center gap-2 text-green-400 text-sm font-medium">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
          Datos cargados
        </span>
        <span class="text-gray-500 text-xs">Recibirás el pago a esta cuenta cuando se confirme cada clase.</span>
        <router-link to="/profile/refund-method" class="btn-secondary text-xs !py-1 !px-3 ml-auto">
          Editar
        </router-link>
      </div>
      <div v-else class="space-y-3">
        <p class="text-gray-300 text-sm">
          Carga tus datos bancarios para poder <strong class="text-white">publicar clases</strong> y recibir tus pagos.
        </p>
        <router-link to="/profile/refund-method" class="btn-primary inline-block">
...
// Datos bancarios (para el desembolso al confirmarse cada clase)
